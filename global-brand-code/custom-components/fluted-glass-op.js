...
function initializeOptimizedShaders() {
    ...
    // Skip on software WebGL (PageSpeed/headless Chrome uses SwiftShader — no real GPU).
    // Catches real users on software renderers where the shader would run poorly
    try {
        ...
        if (_gl) {
            ...
            if (_ext) {
                const _renderer = _gl.getParameter(_ext.UNMASKED_RENDERER_WEBGL);
                if (/SwiftShader|llvmpipe|softpipe|Microsoft Basic/i.test(_renderer)) { return; }
            }
        }
    } catch(e) {}
    ...
    function parseColorValue(colorString, element) {
        ...
        if (colorString.startsWith('var(')) {
            const match = colorString.match(/var\(\s*(--[^,\)]+)\s*(?:,\s*([^)]+))?\s*\)/);
            if (match) {
                ...
                if (!resolvedValue) {
                    ...
                    const variations = [
                        ...
                        `--${baseName.replace(/([A-Z])/g, letter => letter.toLowerCase())}`
                    ];
    ...
    // One shared mousemove listener instead of one per instance
    const mouseMoveHandlers = new Set();
    document.addEventListener('mousemove', (e) => {
        for (const handler of mouseMoveHandlers) handler(e);
    });
    ...
    window.addEventListener('scroll', () => {
        ...
        scrollEndTimeout = setTimeout(() => { isScrolling = false; }, 150);
    }, { passive: true });

    // Opt A: pause rAF loop entirely when tab is hidden
    ...
    function globalAnimate(currentTime) {
        ...
        // At rest: 30fps cap. During scroll: every frame so canvases stay in sync.
        const shouldRender = isScrolling || (currentTime - lastGlobalTime >= 33);
        if (!shouldRender) return;

        if (activeInstances.size === 0 || isPageHidden) return;

        if (!isScrolling) lastGlobalTime = currentTime;
    ...
    async function loadBackgroundTexture(imageUrl) {
        return new Promise((resolve) => {
            if (!imageUrl) { resolve({ texture: null, aspect: 1.0 }); return; }
            ...
            testImg.onload = function() {
                ...
                }, undefined, () => resolve({ texture: createFallbackTexture(), aspect: 1.0 }));
            };
            testImg.onerror = () => resolve({ texture: createFallbackTexture(), aspect: 1.0 });
    ...
    async function createHeavyBlurTexture(textureData) {
        return new Promise((resolve) => {
            ...
            // Opt B: pre-blur on CPU for ALL browsers including Safari.
            // Safari doesn't support ctx.filter, so we use downscale→upscale interpolation
            try {
                ...
                tempImg.onload = function() {
                    ...
                    const supportsCtxFilter = typeof blurCtx.filter === 'string';
                    if (supportsCtxFilter) {
                        // Chrome, Firefox, modern Safari: proper Gaussian blur (GPU-accelerated on real hardware)
                        blurCtx.filter = 'blur(15px)';
                        blurCtx.drawImage(tempImg, 0, 0, 512, 512);
                    } else {
                        // Older Safari/iOS fallback: downscale→upscale interpolation
                        const smallCanvas = document.createElement('canvas');
                        smallCanvas.width = 32;
                        smallCanvas.height = 32;
                        const smallCtx = smallCanvas.getContext('2d');
                        smallCtx.imageSmoothingEnabled = true;
                        smallCtx.drawImage(tempImg, 0, 0, 32, 32);
                    ...
                    const loader = new THREE.TextureLoader();
                    loader.load(blurCanvas.toDataURL(), (blurredTexture) => {
                ...
                tempImg.onerror = () => resolve({ texture: originalTexture, aspect: originalAspect });
            ...
            try {
                switch(step) {
                    case 0:
                        ...
                        state.backgroundTextureData = await loadBackgroundTexture(state.settings.backgroundImage);
                        state.blurredBackgroundData = await createHeavyBlurTexture(state.backgroundTextureData);
                    ...
                    case 1:
                        ...
                        // Per-instance renderer - preserves scroll behavioure in DOM
                        ...
                        state.renderer.setSize(
                            Math.floor(container.clientWidth * perfConfig.resolutionScale),
                            Math.floor(container.clientHeight * perfConfig.resolutionScale),
                            false
                        );
                    ...
                    case 2:
                        state.uniforms = {
                            ...
                            u_color_one: { value: new THREE.Color(parseColorValue(container.getAttribute('data-color-one') || '#5983f8', container) || '#5983f8') },
                            u_size_one: { value: parseFloat(container.getAttribute('data-size-one')) || 0.7 },
                            u_color_two: { value: new THREE.Color(parseColorValue(container.getAttribute('data-color-two') || '#c1ff5b', container) || '#c1ff5b') },
                            ...
                            u_color_three: { value: new THREE.Color(parseColorValue(container.getAttribute('data-color-three') || '#ffff5b', container) || '#ffff5b') },
                            ...
                            u_background_texture: { value: state.blurredBackgroundData.texture },
                            u_has_background: { value: state.blurredBackgroundData.texture !== null },
                        ...
                        state.material = new THREE.ShaderMaterial({
                            ...
                            void main() {
                                ...
                                float alpha = hasAnyBackground ? u_alpha : maxIntensity * u_alpha;
                    ...
                    case 3:
                        ...
                        // Change #5: pre-allocate parallax targets to avoid per-frame Vector2 allocations
                        const parallaxTargets = {
                            b1: new THREE.Vector2(),
                            b2: new THREE.Vector2(),
                            b3: new THREE.Vector2()
                        };
                        ...
                        const instanceController = {
                            update: (time) => {
                                ...
                                // Blob animation + fade: throttle at 30fps independent of render rate
                                if (time - animState.lastRenderTime >= 33) {
                                    ...
                                    if (animState.fadeProgress < 1.0 && animState.isVisible) {
                                        ...
                                        state.uniforms.u_alpha.value = animState.fadeProgress;
                                    }
                                    ...
                                    if (prefersReducedMotion) {
                                        ...
                                        defaultTargets.b1.x = 0.25 + Math.sin(slowTime * 0.7) * 0.05 + Math.cos(fastTime * 0.3) * 0.03;
                                        defaultTargets.b1.y = 0.6 + Math.cos(slowTime * 0.9) * 0.04 + Math.sin(mediumTime * 0.5) * 0.02;

                                        defaultTargets.b2.x = 0.45 + Math.cos(mediumTime * 0.8) * 0.06 + Math.sin(slowTime * 0.4) * 0.0025;
                                        defaultTargets.b2.y = 0.3 + Math.sin(mediumTime * 0.6) * 0.045 + Math.cos(fastTime * 0.7) * 0.02;

                                        defaultTargets.b3.x = 0.75 + Math.sin(fastTime * 0.5) * 0.055 + Math.cos(slowTime * 0.8) * 0.03;
                                        defaultTargets.b3.y = 0.7 + Math.cos(fastTime * 0.4) * 0.04 + Math.sin(slowTime * 0.6) * 0.025;
                                    }
                                    ...
                                    if (state.settings.hoverEnabled && animState.isHovering) {
                                        ...
                                        const parallax1X = offsetX * state.settings.sensitivityOne * state.settings.hoverIntensity;
                                        const parallax1Y = offsetY * state.settings.sensitivityOne * state.settings.hoverIntensity;

                                        const parallax2X = offsetX * -state.settings.sensitivityTwo * state.settings.hoverIntensity;
                                        const parallax2Y = offsetY * -state.settings.sensitivityTwo * state.settings.hoverIntensity;

                                        const parallax3X = offsetX * state.settings.sensitivityThree * state.settings.hoverIntensity;
                                        const parallax3Y = offsetY * state.settings.sensitivityThree * state.settings.hoverIntensity;

                                        parallaxTargets.b1.set(defaultTargets.b1.x + parallax1X, defaultTargets.b1.y + parallax1Y);
                                        parallaxTargets.b2.set(defaultTargets.b2.x + parallax2X, defaultTargets.b2.y + parallax2Y);
                                        parallaxTargets.b3.set(defaultTargets.b3.x + parallax3X, defaultTargets.b3.y + parallax3Y);

                                        blobs.b1.lerp(parallaxTargets.b1, 0.04);
                                        blobs.b2.lerp(parallaxTargets.b2, 0.03);
                                        blobs.b3.lerp(parallaxTargets.b3, 0.02);
                                ...
                                if (animState.fadeProgress <= 0.0) return;

                                state.renderer.render(state.scene, state.camera);
                            },
                            refreshColors: () => {
                                const bgCol = container.getAttribute('data-bg-color');
                                if (bgCol) {
                                    const parsed = parseColorValue(bgCol, container);
                                    if (parsed) state.uniforms.u_background_color.value.set(parsed);
                                }
                                const c1 = container.getAttribute('data-color-one');
                                if (c1) { const p = parseColorValue(c1, container); if (p) state.uniforms.u_color_one.value.set(p); }
                                const c2 = container.getAttribute('data-color-two');
                                if (c2) { const p = parseColorValue(c2, container); if (p) state.uniforms.u_color_two.value.set(p); }
                                const c3 = container.getAttribute('data-color-three');
                                if (c3) { const p = parseColorValue(c3, container); if (p) state.uniforms.u_color_three.value.set(p); }
                            },
                            setVisible: (visible) => {
                                if (visible && !animState.isVisible) {
                                    activeInstances.add(instanceController);
                                    animState.fadeProgress = 0.0;
                                    state.uniforms.u_alpha.value = 0.0;
                        ...
                        if (state.settings.hoverEnabled) {
                            // Use bounds-check inside the shared mousemove handler instead of
                            // mouseenter/mouseleave. 
                            mouseMoveHandlers.add((e) => {
                                const rect = container.getBoundingClientRect();
                                const isOver = e.clientX >= rect.left && e.clientX <= rect.right &&
                                               e.clientY >= rect.top  && e.clientY <= rect.bottom;
                                if (isOver !== animState.isHovering) {
                                    animState.isHovering = isOver;
                                    if (!isOver) mousePos.set(0.5, 0.5);
                                }
                                if (isOver) {
    ...
    function processQueue() {
        ...
        initShader_StepByStep(containerToInit, (controller) => {
            ...
            // Yield to browser between instances so TBT measurement window can close.
            // requestIdleCallback lets the browser become interactive first (Safari falls back to setTimeout).
    ...
    function refreshAllColors() {
        containers.forEach(c => {
            if (c.shaderController) c.shaderController.refreshColors();
        });
    }

    // Webflow theme buttons (data-mode-button="light/dark/auto") — most direct trigger.
    document.querySelectorAll('[data-mode-button]').forEach(btn => {
        btn.addEventListener('click', () => setTimeout(refreshAllColors, 50));
    });

    // Fallback: MutationObserver in case theme is applied via class/attr on <html>
    new MutationObserver(refreshAllColors).observe(document.documentElement, {
        attributes: true,
        attributeFilter: ['class', 'data-theme', 'data-wf-theme']
    });

    // System dark/light mode toggle
    window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', refreshAllColors);
}