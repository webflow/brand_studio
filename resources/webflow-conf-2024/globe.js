  ...
  (function() {
    ...
    for(let i = -9; i <= 9; i++) {
      ...
      for(let j = 0; j <= segments; j++) {
        ...
        const radius_at_latitude = (radius + gridOffset) * Math.cos((i * 10) * Math.PI / 180);
      ...
      const tubeGeometry = new THREE.TubeGeometry(curve, tubeSegments, tubeRadius, tubeRadialSegments, true);
    ...
    for(let i = 0; i < 24; i++) {
      ...
      for(let j = 0; j <= 180; j++) {
        ...
        const x = (radius + gridOffset) * Math.sin(phi) * Math.cos(i * 15 * Math.PI / 180);
        const y = (radius + gridOffset) * Math.cos(phi);
        const z = (radius + gridOffset) * Math.sin(phi) * Math.sin(i * 15 * Math.PI / 180);
        points.push(new THREE.Vector3(x, y, z));
      }
      const curve = new THREE.CatmullRomCurve3(points);
      const tubeGeometry = new THREE.TubeGeometry(curve, tubeSegments, tubeRadius, tubeRadialSegments, false);
      const tube = new THREE.Mesh(tubeGeometry, tubeMaterial);
      sphere.add(tube);
    }
    ...
    const dotSegments = 8;
    const dotGeometry = new THREE.SphereGeometry(dotRadius, dotSegments, dotSegments);
    ...
    for(let i = 0; i < settings.dots.count; i++) {
      ...
      if (Math.random() < settings.dots.clusterProbability) {
        ...
        const center = populationCenters[Math.floor(Math.random() * populationCenters.length)];
    ...
    function triggerAnimation() {
      ...
      setTimeout(() => {
        if (!isMouseDown) { // Only return to base if mouse isn't held
    ...
    function animate() {
      ...
      currentRotationSpeed += (targetRotationSpeed - currentRotationSpeed) * settings.rotation.easing;
    ...
    function regenerateDots() {
      ...
      sphere.children = sphere.children.filter(child => 
                                               !(child.geometry instanceof THREE.SphereGeometry && child.material === dotMaterial)
                                              );
      ...
      for(let i = 0; i < settings.dots.count; i++) {
        ...
        if (Math.random() < settings.dots.clusterProbability) {
          const center = populationCenters[Math.floor(Math.random() * populationCenters.length)];
          const spreadFactor = center.spread * (0.4 + Math.random() * 0.45) * settings.dots.spreadMultiplier;
          lat = center.lat + (Math.random() - 0.5) * spreadFactor;
          lng = center.lng + (Math.random() - 0.5) * spreadFactor;
          lat = Math.max(-60, Math.min(60, lat));
        } else {
