$(function () {
    ...
    for (var i = 0; i < totalGuidelines; i++) {
        var nameWhere = $('.guidelines-wrapper_where .guidelines-reference_where').eq(i);
        for (var d = 0; d < $("[merge='where-title']").length; d++) {
            ...
            if (title.text() == nameWhere.text()) {
                ...
                var titleContain = title.closest("[merge='where-group']").find("[merge='where-list']");
                parent.clone(true, true).appendTo(titleContain);
            }
        }

        var nameWhen = $('.guidelines-wrapper_when .guidelines-reference_when').eq(i);
        for (var d = 0; d < $("[merge='when-title']").length; d++) {
            ...
            if (title.text() == nameWhen.text()) {
                ...
                var titleContain = title.closest("[merge='when-group']").find("[merge='when-list']");
                parent.clone(true, true).appendTo(titleContain);
            }
        }

        var nameCategory = $('.guidelines-wrapper_category .guidelines-reference_category').eq(i);
        for (var d = 0; d < $("[merge='category-title']").length; d++) {
            ...
            if (title.text() == nameCategory.text()) {
                ...
                var titleContain = title.closest("[merge='category-group']").find("[merge='category-list']");
                parent.clone(true, true).appendTo(titleContain);
            }
        }
    }
    ...
    const updateCheckmarks = (checklistItemId, checked) => {
        $("input[data-url-id='" + checklistItemId + "']").each((i, ele) => {
            ...
            if (sectionEle) {
                ...
                const totalChecksinSectionText = sectionEle.find(".checks-counter_total").text();
    ...
    const overrideCheckedFromStorageWithURL = (checkedParamIds) => {
        ...
        updateTotalChecked();
        window.localStorage.setItem('wf_accessiblity_checklist_ids', JSON.stringify(checkedIdsArr));
    }
...
//Toggle task accordions and total progress attributes and visibility

$('.checks-progress_collapse-btn').click(function () {
    $(this).toggleAttrVal('aria-expanded', "true", "false");
});

$('.checks-accordion_trigger-overlay').click(function () {
    $(this).toggleAttrVal('aria-expanded', "false", "true"); //toggle trigger attribute
    $(this).closest('.checks-accordion_trigger').siblings('.accordion-content').toggleAttrVal('aria-hidden', "true", "false"); //toggle content attribute
    $(this).closest('.checks-accordion_trigger').siblings('.accordion-content').toggleClass('cc-show');
    $(this).closest('.checks-accordion_trigger').find('.accordion-icon_left').toggleClass('cc-close');
    $(this).closest('.checks-accordion_trigger').find('.accordion-icon_right').toggleClass('cc-close');
});
